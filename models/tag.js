...
Tag.getTagList = function (params,callback) {
    mysql.pool.getConnection(function(err, connection) {
        ...
        console.log("数据库连接成功！");
        var sql = "SELECT T.tagId, T.tagName, T.tagDescription ,COUNT(A.artTag) AS artNum FROM tag AS T LEFT JOIN article AS A ON A.artTag = T.tagId GROUP BY T.tagName limit ?,?;";
        connection.query(sql, params, function(err, results) {
            if (err) {
                callback(true);
                return;
